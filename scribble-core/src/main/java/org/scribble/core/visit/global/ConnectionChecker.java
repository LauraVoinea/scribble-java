...
public class ConnectionChecker extends STypeVisitor<Global, GSeq> {
    ...
    public SType<Global, GSeq> visitDirectedInteraction(
        ...
        if (n instanceof ConnectAction<?, ?>) {
            switch (this.conns.get(rs)) {
                case AMBIG:
                    throw new ScribException("Roles may already be connected: " + n);
                case CONN:
                    throw new ScribException("Roles already connected: " + n);
                default:
                    this.conns.put(rs, Status.CONN);
            }
        } else {
            switch (this.conns.get(rs)) {
                case AMBIG:
                    throw new ScribException("Roles may be disconnected: " + n);
                case DIS:
                    throw new ScribException("Roles disconnected: " + n);
                default:
            }
        }
    ...
    public SType<Global, GSeq> visitDisconnect(DisconnectAction<Global, GSeq> n)
        ...
        switch (this.conns.get(rs)) {
            case AMBIG:
                throw new ScribException("Roles may already be disconnected: " + n);
            case DIS:
                throw new ScribException("Roles already disconnected: " + n);
            default:
                this.conns.put(rs, Status.DIS);
        }
