...
public class SStateErrors {
    ...
    public final Map<Role, ERecv<DynamicActionKind>> stuck;         // Reception errors
    public final Set<Set<Role>> waitFor;         // Deadlock cycles
    public final Map<Role, Set<ESend<DynamicActionKind>>> orphans;  // Orphan messages
    ...
    public String toErrorMessage(SGraph graph) {
        ...
        if (!isEmpty()) {
            ...
            List<SAction<StaticActionKind>> trace = graph.getTraceFromInit(this.state);  // CHECKME: getTrace broken on non-det self loops?
