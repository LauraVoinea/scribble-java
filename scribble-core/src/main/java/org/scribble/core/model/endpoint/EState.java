...
public class EState extends MPrettyState<RecVar, EAction<StaticActionKind>, EState, Local> {
    ...
    private EState unfairCloneSubgraph(ModelFactory mf, EState term, EAction<StaticActionKind> a, EState succ) // Need succ param for non-det
    {
        Map<Integer, EState> clones = new HashMap<>();  // Original s.id -> clone
        Function<EState, EState> getClone = x ->
        {
        ...
        for (EState s : all.values())  // Quadratic after getReachableStates, but simpler code for now
        {
            ...
            while (as.hasNext()) {
                EAction<StaticActionKind> na = as.next();
    ...
    */
    ...
    private void expandUnfairCases(ModelFactory mf, EState term, Set<EState> todo,
        ...
        if (!aclones.isEmpty())  // Redundant (toRemove would also be empty), but more clear
        {
            ...
            Iterator<EAction<StaticActionKind>> iaclones = aclones.iterator();
    ...
    public void traverse(EStateVisitor v) throws ScribException {
        ...
        // "visitChildren"
        Iterator<EAction<StaticActionKind>> as = this.actions.iterator();
    ...
    protected void visitState(EStateVisitor v) throws ScribException {
        ...
        switch (kind) {
            ...
            case OUTPUT:
                ...
                break;
            case POLY_RECIEVE:
                v.visitPolyInput(this);
                break;
            case SERVER_WRAP:
                v.visitServerWrap(this);
                break;
            case TERMINAL:
                v.visitTerminal(this);
                break;
            case UNARY_RECEIVE:
                v.visitUnaryInput(this);
                break;
            default:
                throw new RuntimeException("Unknown state kind: " + kind);
        }
    }
