...
public class SModel {
    ...
    public void validate(Core core) throws ScribException {
        ...
        if (!core.config.hasArg(CoreArgs.NO_PROGRESS)) {
    ...
    protected Set<Role> checkRoleProgress(Set<SState> termset) {
        ...
        return todo.stream()
                .filter(x -> !s.config.canSafelyTerminate(x)
                        ...
                        .allMatch(y -> y.isEmpty()))  // Check empty queues for starved -- o/w, is a stuck-message
    ...
    protected String termSetToString(Set<SState> termset) {
        return this.core.config.hasArg(CoreArgs.VERBOSE)
