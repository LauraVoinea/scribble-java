...
public class EGraphBuilderUtil
    ...
    @Override
    public void addEdge(EState s, EAction<StaticActionKind> a, EState succ) {
        super.addEdge(s, a, succ);
        for (Deque<List<EAction<StaticActionKind>>> ens : this.collecting.values()) {
            List<EAction<StaticActionKind>> tmp = ens.peek();  // enterRecursion always adds an initial List to the Deque
            if (tmp.isEmpty()) {
                tmp.add(a);
            }
        }
     ...
     */
    public void fixContinueEdge(EState pred, EAction<StaticActionKind> a, EState curr, RecVar rv) {
