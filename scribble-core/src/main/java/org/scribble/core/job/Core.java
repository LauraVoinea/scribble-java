...
public class Core {
    ...
    protected void runProjectionSyntaxWfPasses() throws ScribException {
        ...
        verbosePrintPass("Checking unbounded recursive outputs on all projected inlineds...");
        for (ProtoName<Global> fullname : this.context.getParsedFullnames()) {
            GProtocol inlined = this.context.getInlined(fullname);
            if (inlined.isAux()) {
                continue;
    ...
    protected void runEfsmBuildingPasses() {
        ...
        if (!this.config.hasFlag(CoreArgs.FAIR)) {
            verbosePrintPass(
    ...
    public boolean isVerbose() {
        return this.config.hasFlag(CoreArgs.VERBOSE);
    }
