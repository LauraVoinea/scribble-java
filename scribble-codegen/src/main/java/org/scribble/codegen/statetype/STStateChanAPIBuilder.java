...
public abstract class STStateChanAPIBuilder {
    ...
    public Map<String, String> build()  // filepath -> source
    {
        ...
        // FIXME: sort order
        Map<Integer, EState> states = new LinkedHashMap<>();
        states.put(this.graph.init.id, this.graph.init);
        states.putAll(this.graph.init.getReachableStates());
        ...
        for (EState s : states.values()) {
            ...
            switch (s.getStateKind()) {
                ...
                case OUTPUT:
                    api.put(getFilePath(getStateChanName(s)), this.ob.build(this, s));
                    break;
                case POLY_RECIEVE: {
                ...
                case TERMINAL:
                    api.put(getFilePath(getStateChanName(s)), this.eb.build(this, s));
                    break;  // FIXME: without subpackages, all roles share same EndSocket
                case UNARY_RECEIVE: {
    ...
    public abstract String buildAction(STActionBuilder ab, EState curr, EAction a);  // FIXME: missing type params
