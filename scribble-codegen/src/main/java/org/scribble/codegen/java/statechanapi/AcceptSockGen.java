...
public class AcceptSockGen extends ScribSockGen {
    ...
    protected void addMethods() {
        ...
        List<EAction<StaticActionKind>> as = curr.getDetActions();
        if (as.size() > 1) {
            throw new RuntimeException("AcceptSocket generation not yet supported for accept-branches: " + as);
        }
        EAction<StaticActionKind> a = as.iterator().next();
        EState succ = curr.getDetSuccessor(a);
        makeAcceptMethod(a, succ);
    }

    private void makeAcceptMethod(EAction<StaticActionKind> a, EState succ) {
    ...
    private MethodBuilder makeAcceptHeader(EAction<StaticActionKind> a, EState succ) {
    ...
    public static void setAcceptHeaderWithoutReturnType(StateChannelApiGenerator apigen, EAction<StaticActionKind> a, MethodBuilder mb) {
