...
public class IOInterfacesGenerator extends ApiGen {
    ...
    private final Map<EAction<StaticActionKind>, InterfaceBuilder> actions = new HashMap<>();
    private final Map<EAction<StaticActionKind>, InterfaceBuilder> succs = new HashMap<>();
    private final Map<String, InterfaceBuilder> iostates = new HashMap<>();  // Key is interface simple name

    private final Map<EAction<StaticActionKind>, InterfaceBuilder> caseActions = new HashMap<>();

    private final Map<EState, Set<EAction<StaticActionKind>>> preActions = new HashMap<>();  // Pre set: the actions that lead to each state
    private final Map<EState, Set<InterfaceBuilder>> preds = new HashMap<>();

    private final Map<EState, Set<EAction<StaticActionKind>>> branchPostActions = new HashMap<>();
    //private final Map<EndpointState, Set<InterfaceBuilder>> branchSuccs = new HashMap<>();
    private final Map<String, List<EAction<StaticActionKind>>> branchSuccs = new HashMap<>();  // key: HandleInterface name  // Sorted when collected
    ...
    public IOInterfacesGenerator(StateChannelApiGenerator apigen,
        ...
        //if (IOInterfacesGenerator.skipIOInterfacesGeneration(init))
        {
            ...
            Set<EAction<StaticActionKind>> as = init.getReachableActions();
    ...
    private void generateActionAndSuccessorInterfacesAndCollectPreActions(Set<EState> visited, EState s) throws ScribException {
        ...
        for (EAction<StaticActionKind> a : as.stream().sorted(IOStateIfaceGen.IOACTION_COMPARATOR).collect(Collectors.toList())) {
    ...
    private void generateIOStateInterfacesFirstPass(Set<EState> visited, EState s) throws ScribException {
        ...
        visited.add(s);
        for (EAction<StaticActionKind> a : s.getDetActions()) {
    ...
    private void generateHandleInterfacesSecondPass(Set<EState> visited, EState s) {
        ...
        if (s.getStateKind() == EStateKind.POLY_RECIEVE) {
            ...
            List<EAction<StaticActionKind>> succifs = this.branchSuccs.get(key);
            if (succifs != null) {
    ...
    private void addIOStateInterfacesToStateChannels(Set<EState> visited, EState s) {
        ...
        if (s.getStateKind() == EStateKind.POLY_RECIEVE) {
            ...
            for (EAction<StaticActionKind> a : s.getDetActions().stream().sorted(IOStateIfaceGen.IOACTION_COMPARATOR).collect(Collectors.toList())) {
                if (first) {
                    first = false;
                } else {
                    tmp += ", ";
    ...
    private void putPreAction(EState s, EAction<StaticActionKind> a) {
        putMapSet(this.preActions, s, a);
    }

    private void putBranchPostAction(EState s, EAction<StaticActionKind> a) {
    ...
    private void collectPreds() {
        for (EState s : this.preActions.keySet()) {
            ...
            for (EAction<StaticActionKind> a : this.preActions.get(s))  // sort?
            {
                if (!a.isSend() && !a.isReceive())  // TODO (connect/disconnect)
                {
                    throw new RuntimeException("TODO: " + a);
    ...
    private void collectBranchSuccs() {
        ...
        for (EState s : this.branchPostActions.keySet()) {
            ...
            List<EAction<StaticActionKind>> curr1 = new LinkedList<>();
            ...
            List<EAction<StaticActionKind>> tmp = this.branchSuccs.get(key);
            if (tmp == null) {
                tmp = new LinkedList<>();
                //this.branchSuccs.put(key, tmp);
                tmp.addAll(curr1);
            } else {
                for (EAction<StaticActionKind> a : curr1) {
