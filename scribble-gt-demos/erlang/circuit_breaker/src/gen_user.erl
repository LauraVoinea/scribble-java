...
-callback s8(EventType :: term(), {pid(), {term()}, integer()}, state_data()) -> {stop, normal, state_data()}.
...
start_link(CallbackModule, Args) ->
    case code:ensure_loaded(CallbackModule) of
        {module, CallbackModule} ->
            gen_statem:start_link({local, CallbackModule}, gen_user, {CallbackModule, Args},
              [{debug, [trace, {log_to_file, "user_debug.log"}]}]);
        {error, Reason} ->
            {error, Reason}
    end.
...
-spec s8(EventType :: term(), {pid(), {term()}, integer()}, state_data()) -> {stop, normal, state_data()}.
...
s8(_EventType, {_Pid, Msg, _Counter}, Data) when Msg =:= {error_response} 
    ...
    io:format("gen_user: Garbage collecting event ~p~n", [Msg]),
    {keep_state, Data}.
