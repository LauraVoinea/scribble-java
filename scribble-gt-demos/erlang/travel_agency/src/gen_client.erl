...
-export([init/1, callback_mode/0, code_change/4, terminate/3, start_link/2, send_s3_booking_request/3, s3/3, s4/3, send_s9_accept_offer/2, s9/3, send_s9_resubmit_request/2, send_s9_reject_offer/2, s10/3, send_s11_provide_address/3, s11/3, s12/3, s14/3, send_s15_cancel_booking/2, s15/3, s17/3, send_s18_resubmitting/2, s18/3, send_s6_cancel_agency/2, s6/3, send_s7_cancel_supplier/2, s7/3]).
...
-callback s9(EventType :: term(), {pid(), {term()}, integer()} | term(), state_data()) -> {next_state, s10, state_data()} | {next_state, s17, state_data()} | {next_state, s14, state_data()} | {keep_state, state_data()}.
...
-callback s14(EventType :: term(), term(), state_data()) -> {keep_state, state_data()}.
-callback s17(EventType :: term(), term(), state_data()) -> {keep_state, state_data()}.
...
start_link(CallbackModule, Args) ->
    case code:ensure_loaded(CallbackModule) of
        {module, CallbackModule} ->
            gen_statem:start_link({local, CallbackModule}, gen_client, {CallbackModule, Args}, 
            % []);
            [{debug, [trace, {log_to_file, "client_debug.log"}]}]);
        {error, Reason} ->
            {error, Reason}
    end.
...
-spec s9(EventType :: term(), {pid(), {term()}, integer()} | term(), state_data()) -> {next_state, s10, state_data()} | {next_state, s17, state_data()} | {next_state, s14, state_data()} | {keep_state, state_data()}.
...
s9(EventType, {AgencyPid, {price_adjustment, Price}, Counter}, #state_data{mc_counter_1 = MC} = Data) when Counter =:= MC ->
    CallbackModule = get(callback_module),
    CallbackModule:s9(EventType, {AgencyPid, {price_adjustment, Price}}, Data);
s9(_EventType, {_Pid, Msg, _Counter}, Data) when Msg =:= {repeat_confirmation} 
...
s10(_EventType, {_Pid, Msg, _Counter}, Data) when Msg =:= {repeat_confirmation} 
		orelse Msg =:= {accept_confirmation} 
		orelse Msg =:= {reject_confirmation} ->
    {keep_state, Data};
...
s12(_EventType, {_Pid, Msg, _Counter}, Data) when Msg =:= {repeat_confirmation} 
		orelse Msg =:= {accept_confirmation} 
		orelse Msg =:= {reject_confirmation} ->
    {keep_state, Data};
s12(_EventType, {_Pid, {price_quote, _Price}, _Counter}, Data) ->
    {keep_state, Data};
s12(_EventType, {_Pid, {price_adjustment, _Price}, _Counter}, Data) ->
    {keep_state, Data};
s12(_EventType, {_Pid, {confirm_date, _Date}, _Counter}, Data) ->
    {keep_state, Data}.
...
-spec s14(EventType :: term(), term(), state_data()) -> {keep_state, state_data()}.
...
s14(_EventType, {_Pid, Msg, _Counter}, Data) when Msg =:= {repeat_confirmation} 
		orelse Msg =:= {accept_confirmation} 
		orelse Msg =:= {reject_confirmation} ->
    {keep_state, Data};
s14(_EventType, {_Pid, {price_quote, _Price}, _Counter}, Data) ->
    {keep_state, Data};
s14(_EventType, {_Pid, {price_adjustment, _Price}, _Counter}, Data) ->
    {keep_state, Data};
s14(_EventType, {_Pid, {confirm_date, _Date}, _Counter}, Data) ->
    {keep_state, Data}.

-spec s17(EventType :: term(), term(), state_data()) -> {keep_state, state_data()}.
...
s17(_EventType, {_Pid, Msg, _Counter}, Data) when Msg =:= {repeat_confirmation} 
		orelse Msg =:= {accept_confirmation} 
		orelse Msg =:= {reject_confirmation} ->
    {keep_state, Data};
s17(_EventType, {_Pid, {price_quote, _Price}, _Counter}, Data) ->
    {keep_state, Data};
