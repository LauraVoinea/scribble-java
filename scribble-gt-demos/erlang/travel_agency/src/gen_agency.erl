...
-export([init/1, callback_mode/0, code_change/4, terminate/3, start_link/2, s3/3, send_s4_price_quote/3, s4/3, send_s8_price_adjustment/3, s8/3, send_s9_accept_confirmation/2, s9/3, send_s11_reject_confirmation/2, s11/3, send_s13_repeat_confirmation/2, s13/3, s6/3]).
...
-callback s6(term() | EventType :: term(), {pid(), {atom(), term()}} | term(), state_data()) -> {stop, normal, state_data()} | {keep_state, state_data()}.
-callback s13(EventType :: term(), {atom()}, state_data()) -> {ok, s3, state_data()}.
-callback s8(EventType :: term(), {atom()} | term(), state_data()) -> {next_state, s6, state_data()} | {keep_state, state_data()}.
...
start_link(CallbackModule, Args) ->
    case code:ensure_loaded(CallbackModule) of
        {module, CallbackModule} ->
            ...
            % []);
            [{debug, [trace, {log_to_file, "agency_debug.log"}]}]);
        {error, Reason} ->
            {error, Reason}
    end.
...
-spec s6(term() | EventType :: term(), {pid(), {atom(), term()}} | term(), state_data()) -> {stop, normal, state_data()} | {keep_state, state_data()}.
...
-spec s8(EventType :: term(), {atom()} | term(), state_data()) -> {next_state, s6, state_data()} | {keep_state, state_data()}.
