...
-spec s33(EventType :: term(), {pid(), {term()}, integer()}, state_data()) ->
    ...
    {keep_state, state_data(), [postpone]} |
    {stop, normal, state_data()}.
s33(_EventType, {_Pid, {'221'}, Counter}, #state_data{mc_counter_2 = MC} = Data) when Counter >= MC ->
    io:format("gen_c: Postponing event ~p~n", [['221']]),
    {keep_state, Data, [postpone]};
s33(_EventType, {_Pid, {'250'}, Counter}, #state_data{mc_counter_1 = MC} = Data) when Counter >= MC ->
    io:format("gen_c: Postponing event ~p~n", [['250']]),
    {keep_state, Data, [postpone]};
s33(_EventType, {_Pid, {'501'}, Counter}, #state_data{mc_counter_2 = MC} = Data) when Counter >= MC ->
    io:format("gen_c: Postponing event ~p~n", [['501']]),
    {keep_state, Data, [postpone]};
s33(_EventType, {_Pid, {'354'}, Counter}, #state_data{mc_counter_1 = MC} = Data) when Counter >= MC ->
...
-spec s5(EventType :: term(), {pid(), {term()}, integer()}, state_data()) ->
    ...
    {keep_state, state_data(), [postpone]} |
    {stop, normal, state_data()}.
s5(_EventType, {_Pid, {'Ack'}, Counter}, #state_data{mc_counter_2 = MC} = Data) when Counter >= MC ->
    io:format("gen_c: Postponing event ~p~n", [['Ack']]),
    {keep_state, Data, [postpone]};
s5(_EventType, {_Pid, {'220'}, Counter}, #state_data{mc_counter_2 = MC} = Data) when Counter >= MC ->
    io:format("gen_c: Postponing event ~p~n", [['220']]),
    {keep_state, Data, [postpone]};
s5(_EventType, {_Pid, {'EhloCommit'}, Counter}, #state_data{mc_counter_2 = MC} = Data) when Counter >= MC ->
    io:format("gen_c: Postponing event ~p~n", [['EhloCommit']]),
    {keep_state, Data, [postpone]};
s5(_EventType, {_Pid, {'AckCommit'}, Counter}, #state_data{mc_counter_2 = MC} = Data) when Counter >= MC ->
    io:format("gen_c: Postponing event ~p~n", [['AckCommit']]),
    {keep_state, Data, [postpone]};
s5(_EventType, {_Pid, {'535'}, Counter}, #state_data{mc_counter_2 = MC} = Data) when Counter >= MC ->
    io:format("gen_c: Postponing event ~p~n", [['535']]),
    {keep_state, Data, [postpone]};
s5(_EventType, {_Pid, {'250d'}, Counter}, #state_data{mc_counter_2 = MC} = Data) when Counter >= MC ->
    io:format("gen_c: Postponing event ~p~n", [['250d']]),
    {keep_state, Data, [postpone]};
s5(_EventType, {_Pid, {'221'}, Counter}, #state_data{mc_counter_2 = MC} = Data) when Counter >= MC ->
    io:format("gen_c: Postponing event ~p~n", [['221']]),
    {keep_state, Data, [postpone]};
s5(_EventType, {_Pid, {'235'}, Counter}, #state_data{mc_counter_2 = MC} = Data) when Counter >= MC ->
    io:format("gen_c: Postponing event ~p~n", [['235']]),
    {keep_state, Data, [postpone]};
s5(_EventType, {_Pid, {'501'}, Counter}, #state_data{mc_counter_2 = MC} = Data) when Counter >= MC ->
    io:format("gen_c: Postponing event ~p~n", [['501']]),
    {keep_state, Data, [postpone]};
s5(_EventType, {_Pid, {'250'}, Counter}, #state_data{mc_counter_1 = MC} = Data) when Counter >= MC ->
    io:format("gen_c: Postponing event ~p~n", [['250']]),
    {keep_state, Data, [postpone]};
s5(_EventType, {_Pid, {'250'}, Counter}, #state_data{mc_counter_2 = MC} = Data) when Counter >= MC ->
    io:format("gen_c: Postponing event ~p~n", [['250']]),
    {keep_state, Data, [postpone]};
