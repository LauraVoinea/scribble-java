...
-export([init/1, callback_mode/0, code_change/4, terminate/3, start_link/2, s1/3, send_s3_login_success/2, s3/3, send_s3_login_failed/2, send_s4_login_accepted/2, s4/3, s8/3, send_s12_auth_fail/2, s12/3]).

-include("a.hrl").
-type state_data() :: #state_data{mc_counter_1 :: integer(), c_pid :: pid() | undefined, s_pid :: pid() | undefined}.

-callback s3(EventType :: term(), {atom()}, state_data()) -> {next_state, s4, state_data(), [{next_event, internal, {login_accepted}}]} | {next_state, s12, state_data(), [{next_event, internal, {auth_fail}}]}.
...
-callback s8(EventType :: term(), {pid(), {term()}, integer()} | term(), state_data()) -> {next_state, s8, state_data()} | {stop, normal, state_data()} | {stop, normal, state_data()} | {keep_state, state_data()}.
-callback s1(term(), {pid(), {atom(), term()}}, state_data()) -> {next_state, s3, state_data()} | {next_state, s3, state_data(), [term()]}.
...
-spec s3(EventType :: term(), {atom()}, state_data()) -> {next_state, s4, state_data(), [{next_event, internal, {login_accepted}}]} | {next_state, s12, state_data(), [{next_event, internal, {auth_fail}}]}.
...
s8(_EventType, {_Pid, Msg, _Counter}, Data) when Msg =:= {timeout} 
    ...
    {keep_state, Data};
s8(_EventType, {_Pid, {login, _Id, _Password}, _Counter}, Data) ->
    {keep_state, Data}.
...
-spec s1(term(), {pid(), {atom(), term(), term()}}, state_data()) -> {next_state, s3, state_data()} | {next_state, s3, state_data(), [term()]}.
