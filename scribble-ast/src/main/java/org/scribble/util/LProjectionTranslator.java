...
public class LProjectionTranslator {
    ...
    protected ProtoModNode translate(ProtoMod mod) {
        switch (mod) {
            case AUX:
                return af.AuxMod(null);
            case EXPLICIT:
                return af.ExplicitMod(null);
            default:
    ...
    protected org.scribble.ast.local.LChoice translate(LChoice t) {
        RoleNode subj = this.af.RoleNode(null, t.getSubject().toString());  // Includes "self"
        List<LProtoBlock> blocks = t.getBlocks().stream().map(x -> translate(x))
                .collect(Collectors.toList());
    ...
    protected org.scribble.ast.local.LRecursion translate(LRecursion t) {
        RecVarNode rv = this.af.RecVarNode(null, t.getRecVar().toString());
        return this.af.LRecursion(null, rv, translate(t.getBody()));
    }
