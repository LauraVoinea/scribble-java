...
public class AssrtLTypeFactory extends LTypeFactoryImpl
{
	...
	public AssrtLChoice AssrtCoreLChoice(CommonTree source, Role peer,
			AssrtLActionKind kind,
			LinkedHashMap<AssrtMsg, AssrtLType> cases)
	{
		return new AssrtLChoice(source, peer, kind, cases);
	}

	public AssrtLRec AssrtCoreLRec(CommonTree source, RecVar rv,
			LinkedHashMap<AssrtVar, AssrtAFormula> svars,
			AssrtLType body, AssrtBFormula bform,
			LinkedHashMap<AssrtVar, AssrtAFormula> phantom)
	{
		return new AssrtLRec(source, rv, body, svars, bform, phantom);
	}

	public AssrtLRecVar AssrtCoreLRecVar(CommonTree source, RecVar rv,
			List<AssrtAFormula> aforms)
	{
		return new AssrtLRecVar(source, rv, aforms);
	}

	public AssrtLEnd AssrtCoreLEnd()
	{
