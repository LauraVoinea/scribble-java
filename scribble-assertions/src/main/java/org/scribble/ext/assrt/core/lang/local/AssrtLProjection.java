...
import org.scribble.ext.assrt.core.model.endpoint.AssrtEGraphBuilder;
...
// N.B. not an AssrtCoreLProtocol ...
// TODO: fix CoreContext G/LProtocol hardcoding
public class AssrtLProjection extends LProjection  //implements AssrtCoreProtocol<...> ?
{
	// TODO: duplicated from AssrtCoreGProtocol -- refactor
	public final AssrtLType type;
	...
	public AssrtLProjection(List<ProtoMod> mods, LProtoName fullname,
			List<Role> rs, Role self, List<MemberName<? extends NonRoleParamKind>> ps,
			GProtoName global, AssrtLType type,
	...
	public LProjection reconstruct(CommonTree source, List<ProtoMod> mods,
			LProtoName fullname, List<Role> rs, Role self,
			List<MemberName<? extends NonRoleParamKind>> ps, AssrtLType type,
			LinkedHashMap<AssrtVar, AssrtAFormula> svars, AssrtBFormula ass,
			LinkedHashMap<AssrtVar, AssrtAFormula> phantom)
	{
		return new AssrtLProjection(mods, fullname, rs, this.self, ps,
	...
	@Override
	public AssrtLProjection getInlined(STypeInliner<Local, LSeq> v)
	{
	...
	{
		if (this.type.equals(AssrtLEnd.END))
		{
			EState s = ((AssrtEModelFactory) core.config.mf.local)
					.EState(Collections.emptySet());
			return new EGraph(s, s);  // TODO: refactor constructor inside mf
		}
		AssrtEGraphBuilder b = (AssrtEGraphBuilder) core.config.vf.local
	...
	public boolean equals(Object o)
	{
		...
		if (!(o instanceof AssrtLProjection))
		{
			return false;
		}
		AssrtLProjection them = (AssrtLProjection) o;
	...
	{
		return o instanceof AssrtLProjection;
	}
}
