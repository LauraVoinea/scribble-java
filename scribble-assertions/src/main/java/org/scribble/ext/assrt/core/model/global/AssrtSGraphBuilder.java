...
public class AssrtSGraphBuilder extends SGraphBuilder 
{
	public AssrtSGraphBuilder(ModelFactory mf)
	{
		super(mf);
	}
	
	@Override
	public AssrtSGraph build(Map<Role, EGraph> egraphs, boolean isExplicit,
			GProtoName fullname)
	{
		...
		while (!todo.isEmpty())
		{
			...
			while (!es.isEmpty())
			{
				...
				for (EAction a : as)
				{
					...
					((AssrtSState) curr).addSubject(self);
					if (succ.size() > 1)
					{
						throw new RuntimeException(
		...
		return (AssrtSGraph) this.mf.global.SGraph(fullname,
