...
public class AssrtEGraphBuilder extends EGraphBuilder
{
	...
	public AssrtEGraphBuilder(AssrtCore core)
	{
		super(core);
		/*this.core = core;
	...
	public EGraph build(LinkedHashMap<AssrtVar, AssrtAFormula> svars,
			AssrtBFormula ass, AssrtLType lt,
			LinkedHashMap<AssrtVar, AssrtAFormula> phantom)  // Phantoms from init state
	{
		this.util.setEntry(((AssrtEModelFactory) this.core.config.mf.local)
	...
	private void build(AssrtLType lt, AssrtEState s1, AssrtEState s2,
	...
	{
		if (lt instanceof AssrtLChoice)
		{
			AssrtLChoice lc = (AssrtLChoice) lt;
			AssrtLActionKind k = lc.getKind();
			lc.cases.entrySet().stream()
					.forEach(e -> buildEdgeAndContinuation(s1, s2, recs, lc.role, k,
							e.getKey(), e.getValue(), phantom, phantAss));
		}
		else if (lt instanceof AssrtLRec)
		{
			AssrtLRec lr = (AssrtLRec) lt;
	...
	private void buildEdgeAndContinuation(AssrtEState s1, AssrtEState s2,
			Map<RecVar, AssrtEState> recs, Role r, AssrtLActionKind k,
			AssrtMsg a, AssrtLType cont,
			LinkedHashMap<AssrtVar, AssrtAFormula> phantom, AssrtBFormula phantAss)
	{
		if (cont instanceof AssrtLEnd)
		{
			this.util.addEdge(s1, toEAction(r, k, a, phantom, phantAss), s2);
		}
		else if (cont instanceof AssrtRecVar)
		{
			AssrtLRecVar rv = (AssrtLRecVar) cont;
		...
		else
		{
			...
			if (cont instanceof AssrtLRec)  // `a` is a f/w rec -- cont-rec `a` handled by RecVar `cont` above
			{
				...
				LinkedHashMap<AssrtVar, AssrtAFormula> svars = ((AssrtLRec) cont).statevars;
	...
	private EAction toEAction(Role r, AssrtLActionKind k, AssrtMsg a,
	...
	*/
			...
			LinkedHashMap<AssrtVar, AssrtAFormula> phantom, AssrtBFormula phantAss)  // Carried over from state
	{
		...
		else if (k.equals(AssrtLActionKind.REQ))
		{
			/*if (!phantom.isEmpty())
			{
		...
		else if (k.equals(AssrtLActionKind.ACC))
		{
			/*if (!phantom.isEmpty())
			{
				throw new RuntimeException("TODO");
