...
public class AssrtSSingleBuffers extends SSingleBuffers
{
	public AssrtSSingleBuffers(Set<Role> roles, boolean implicit)
	{
		super(roles, implicit);
	}
	
	// TODO refactor to modelfactory (cf. super)
	protected AssrtSSingleBuffers()
	{
		
	}
	
	@Override
	protected AssrtSSingleBuffers copy()  // TODO: refactor to modelfactory (cf. super)
	{
		AssrtSSingleBuffers copy = new AssrtSSingleBuffers();
  ...
  // N.B. state args ignored for recv fireable and firing (msgs don't carry state args)
	...
	{
		AssrtESend msg = (AssrtESend) this.buffs.get(self).get(a.peer);
		return isConnected(self, a.peer)  // Other direction doesn't matter, local can still receive after peer disconnected
				&& msg != null && msg//.toTrueAssertion()
						.toDual(a.peer)
						.equals(((AssrtERecv) a).dropStateArgs());   // Cf. AssrtCoreEMsg constructor -- also AssrtSConfig.async -> AssrtCoreESend.toTrueAssertion
	...
	public boolean equals(Object o)
	{
		...
		if (!(o instanceof AssrtSSingleBuffers))
		{
			return false;
		}
	...
	{
		return o instanceof AssrtSSingleBuffers;
	}
}
