...
tokens
{
  ...
  /* Locals: currently not directly parsed, but generated by, e.g., projection */
  LPROTODECL;
...
// Assrt
|
	assrt_varname ':' ambigname
->
	^(ASSRT_ANNOTDATAELEM assrt_varname ambigname)
|
	assrt_varname ':' qualifieddataname
-> 
	^(ASSRT_ANNOTDATAELEM assrt_varname qualifieddataname)
...
->
	^(ASSRT_GPROTOHEADER simplegprotoname ^(PARAMDECL_LIST) roledecls 
			...
			// FIXME: EXTID parsed twice -- how to factor out without changing AssrtGProtoHeader? (Or just refactor the latter?)
;
// Following same pattern as gmsgtransfer: explicitly invoke AssertionsParser, extra assertion element only for new category
...
ginteraction:
 assrt_gmsgtransfer | gdo | gchoice 

// TODO
|
	assrt_gconnect | gwrap | gdisconnect | gcontinue | grecursion
; 
...
assrt_gmsgtransfer:
	//message FROM_KW rolename TO_KW rolename (',' rolename )* ';'  // TODO: multisend
...
// N.B. calling an "external" parser this way loses line/char number information
...
->
	^(GDO gprotoname nonroleargs roleargs)  // Currently converted to AssrtGDo by AssrtScribTreeAdaptor
