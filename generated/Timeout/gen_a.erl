...
-export([init/1, callback_mode/0, code_change/4, terminate/3, start_link/2, send_a1/2, s4/3, s5/3, send_a6/2, s6/3]).
...
-callback s5(EventType :: term(), {pid(), {atom(), term()}} | term(), state_data()) -> {next_state, s6, state_data(), [{next_event, internal, {a6}}]} | {stop, normal, state_data()} | {keep_state, state_data()}.
...
-spec init({CallbackModule :: module(), Args :: list()}) -> {ok, s4, state_data(), [{next_event, internal, {a1}}]}.
...
s4(EventType, {BPid, {'Timeout'}, Counter}, #state_data{mc_counter_1 = MC} = Data) when Counter == MC ->
    CallbackModule = get(callback_module),
    CallbackModule:s4(EventType, {BPid, {'Timeout'}}, Data);
s4(_EventType, {_Pid, _Label, _Counter}, Data) ->
    {keep_state, Data}.

-spec s5(EventType :: term(), {pid(), {atom(), term()}} | term(), state_data()) -> {next_state, s6, state_data(), [{next_event, internal, {a6}}]} | {stop, normal, state_data()} | {keep_state, state_data()}.
s5(EventType, {BPid, {a5}, Counter}, #state_data{mc_counter_1 = MC} = Data) when Counter == MC ->
    CallbackModule = get(callback_module),
    CallbackModule:s5(EventType, {BPid, {a5}}, Data);
s5(EventType, {BPid, {'Timeout'}, Counter}, #state_data{mc_counter_1 = MC} = Data) when Counter == MC ->
    CallbackModule = get(callback_module),
    CallbackModule:s5(EventType, {BPid, {'Timeout'}}, Data);
s5(_EventType, {_Pid, _Label, _Counter}, Data) ->
    {keep_state, Data}.
